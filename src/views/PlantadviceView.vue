  ...
  <div class="plant-advice-container">
    ...
    <div class="content-section">
      ...
      <div class="recommendations-section" v-if="selectedSeason">
        ...
        <div v-if="recommendedPlants.length > 0 && !loading && !error" class="planner-button-container">
          <router-link
            :to="{
        ...
        <div v-else class="plants-grid">
          ...
          >
            ...
            <div class="plant-info">
              ...
              <p>{{ plant.description && plant.description.length > 100 ? plant.description.substring(0, 100) + '...' : plant.description }}</p>
            </div>
          </router-link>
        </div>
      </div>
  ...
  setup() {
           ...
           try {
            ...
            if (nominatimData && nominatimData.length > 0) {
                 suggestions.value = nominatimData.map((item) => ({
                    ...
                    context: item.address && item.address.state ? [{id: 'region.nominatim', text: item.address.state}] : null
                }));
            } else {
                suggestions.value = [];
            }
    ...
    const extractStateFromContext = (contextArray) => {
        ...
        for (const context of contextArray) {
            if (context.id && context.id.startsWith('region.')) {
                ...
                if (stateName.toLowerCase().includes('queensland') || context.short_code?.toLowerCase().includes('qld')) return 'Queensland';
                if (stateName.toLowerCase().includes('victoria') || context.short_code?.toLowerCase().includes('vic')) return 'Victoria';
                if (stateName.toLowerCase().includes('south australia') || context.short_code?.toLowerCase().includes('sa')) return 'South Australia';
            }
        }
        return null;
    };
    ...
    const selectSuggestion = (index) => {
      ...
      if (stateFromSuggestion) {
        ...
        locationError.value = 'Could not determine state from this selection. Please refine or click Search.'
      }
      suggestions.value = []
    }
    ...
    const reverseGeocodeForState = async (lng, lat) => {
        ...
        } catch (err) { console.error('Reverse geocoding for state failed:', err); }
        return null;
    };
    ...
    const searchLocation = async () => {
      ...
      if (selectedState.value && ['Queensland', 'Victoria', 'South Australia'].includes(selectedState.value)) {
        stateFound = true; // State might have been set by selectSuggestion
      }

      // If state not found yet, try to determine it
      if (!stateFound) {
        if (!selectedLocation.value) { // No lat/lon from suggestion, geocode address
            try {
                ...
                if (geoData.features && geoData.features.length > 0) {
                    ...
                    const stateFromGeo = extractStateFromContext(feature.context);
                     if (stateFromGeo && ['Queensland', 'Victoria', 'South Australia'].includes(stateFromGeo)) {
                        selectedState.value = stateFromGeo;
                        localStorage.setItem('selectedState', selectedState.value);
                        stateFound = true;
                    }
                } else { locationError.value = 'Location not found.'; return; }
            } catch (err) {
                console.error("Geocoding error:", err);
                locationError.value = "Error finding location details."; return;
            }
        }
        // If state still not found, but we have lat/lon, reverse geocode
        if (!stateFound && selectedLocation.value) {
            const stateFromReverse = await reverseGeocodeForState(selectedLocation.value.lng, selectedLocation.value.lat);
            if (stateFromReverse && ['Queensland', 'Victoria', 'South Australia'].includes(stateFromReverse)) {
      ...
      if (!stateFound) {
        locationError.value = 'Could not determine a valid state (QLD, VIC, SA) for your location. Please be more specific or select from suggestions.';
        return;
      }
    ...
    const fetchRecommendedPlants = async () => {
      ...
      try {
        ...
        if (plantData && Array.isArray(plantData)) { // API now returns an array
            recommendedPlants.value = plantData; // Directly assign the array
            if (plantData.length === 0) {
              error.value = "No specific plant recommendations found for this selection. You can still browse general advice.";
            }
      ...
      if (selectedState.value && selectedSeason.value) {
        ...
        if (savedPlants) {
          try {
            ...
            } else { // If format is wrong, fetch fresh
                fetchRecommendedPlants();
            }
          } catch (e) {
    ...
    return {
      address, suggestions, selectedLocation, selectedState, selectedSeason,
      recommendedPlants, loading, error, locationError, userHemisphere,
      fetchSuggestions, selectSuggestion, searchLocation, selectSeason,
      getPlantImage, handleImageError,
    }
  },
}
</script>
...
.banner {
    background-image:
        linear-gradient(rgba(0, 50, 0, 0.8), rgba(0, 50, 0, 0.6)),
        url('@/assets/images/garden.jpeg');

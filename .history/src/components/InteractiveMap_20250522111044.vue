  ...
  <div class="interactive-map-layout">
    <div class="top-image-bar">
      <div class="image-item">
        ...
        <p class="image-caption">Migrates between two loccations</p>
      </div>
      <div class="image-item">
        <img src="/images/guide/guide2.png" alt="Image 2" />
        <p class="image-caption">Stays at this location for one month</p>
      </div>
      <div class="image-item">
        <img src="/images/guide/guide3.png" alt="Image 3" />
        <p class="image-caption">Hover to view bird's status</p>
      </div>
      <div class="image-item">
        <img src="/images/guide/guide4.png" alt="Image 4" />
        <p class="image-caption">Click image to view bird's information</p>
      </div>
      <div class="image-item">
    ...
    <div class="map-main">
      <div ref="mapContainer" class="map-section">
        <div class="map-top-right-control">
          <button
            class="toggle-animation-button"
            :disabled="isAnimating"
            @click="toggleAnimationSetting"
          >
            {{ animationEnabled ? 'Animation: ON' : 'Animation: OFF' }}
          </button>
          <button
            class="toggle-path-button"
            :disabled="!selectedSpecies || isAnimating"
      ...
      <div class="sidebar">
        ...
        <div class="species-scroll">
          <label
            v-for="bird in filteredBirdList"
            :key="bird.name"
            class="bird-card"
    ...
    <div class="bird-modal" @click.stop>
      ...
      <div class="bird-modal-header">
        <div class="bird-modal-image-container">
          <img
            :src="getBirdImagePath(selectedBirdInfo.image)"
            :alt="selectedBirdInfo.name"
...
function getDominantStateForSpeciesMonth(species, monthNumber) {
  ...
  const filtered = observationData.filter(
    (entry) => entry.species_common_name === species && entry.month === monthStr,
  )
  const counts = {}
...
function getStateNameFromCoord(coord) {
  const entry = Object.entries(stateCapitals).find(
...
function placeInteractivePointMarkers(species) {
  ...
  Object.values(pointMap).forEach(({ coord, entries }) => {
    ...
    const marker = new mapboxgl.Marker(el).setLngLat(coord).addTo(map.value)
    ...
    el.addEventListener('mouseenter', () => {
      ...
      const content = Object.values(grouped)
        .map((group) => {
...
onMounted(() => {
  try {
    ...
    mapboxgl.accessToken =
      'pk.eyJ1IjoidGxpdTAwNTgiLCJhIjoiY2x1N3B3emZrMDhmaTJ2bnIyeDkzb2lqZyJ9.ILfUF13AATr3Ud-oXgbZeg'

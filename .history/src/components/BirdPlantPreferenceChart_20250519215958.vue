  ...
  setup(props) {
    ...
    onMounted(async () => {
      ...
      if (props.birdsData && props.birdsData.length > 0) {
        ...
        // Wait for DOM update
        await nextTick()
        // Don't call updateChart here - let the watch handle it
      } else {
        console.log('Bird data not yet loaded')
      }
    })

    // Make sure to destroy chart when component is unmounted
    onUnmounted(() => {
      if (chart.value) {
        chart.value.destroy()
        chart.value = null
    ...
    const updateChart = async () => {
      ...
      // First, ensure old chart is destroyed
      if (chart.value) {
        chart.value.destroy()
        chart.value = null // Important: set reference to null
      }
      ...
      async (newValue) => {
        ...
        if (newValue && newValue.length > 0) {
          ...
          if (!selectedBird.value && birds.length > 0) {
            ...
            // If bird is already selected but data changed, we need to update chart
            // Ensure old chart is destroyed first
            if (chart.value) {
              chart.value.destroy()
              chart.value = null
            }
